...
// Interactive Cosmic News Articles
const NEWS_ARTICLES = [
  {
    id: 'n1',
...
MiniVerse explorers are encouraged to load their regional thermal telescopes and reference system coordinate Node 4-7C to witness the lingering ion sparks.`
  },
  {
    ...
    title: 'Superluminal Subspace Link Established with Andromeda Sector Relays',
    time: '3 hours ago',
    views: '118K',
    author: 'Dr. Evelyn Vance, MiniVerse Labs',
    readTime: '6 min read',
    image: 'https://images.unsplash.com/photo-1518770660439-4636190af475?q=80&w=800&auto=format&fit=crop',
    excerpt: 'Quantum entanglement transmitter network successfully completes first continuous zero-latency telemetry sync between dual galaxies.',
    body: `Physicists at the Kepler Central Telemetry Hub have successfully demonstrated a unified network link to Andromeda sector probes using a novel triplet-state entanglement matrix. Unlike standard particle locks, this system operates with double-shielded decoherence protocols, delivering continuous ping-free telemetry sync.

This achievement effectively breaks past the previous distance threshold of light-years, rendering light-speed lag obsolete for deep space probes. Operational tests returned clean binary datasets representing thermal planetary maps of Sector 14-8 without a single parity bit error. 

"We are looking at a future where communication across the cosmic void is as fast as local server queries," stated Dr. Vance during the live-streamed presentation.`
  },
  {
    id: 'n3',
    category: 'Stellar Tech • UPDATE',
    title: 'Dyson Swarm Construction Begins Phase 4 in Solar Coordinates Delta-9',
    time: '6 hours ago',
    views: '95K',
    author: 'Space Infrastructure Syndicate',
    readTime: '3 min read',
    image: 'https://images.unsplash.com/photo-1446776811953-b23d57bd21aa?q=80&w=800&auto=format&fit=crop',
    excerpt: 'Over forty thousand solar sails deployed in perfect grid formation to feed orbital power grids.',
    body: `The Energy Syndicate of the Core Worlds announced the successful deployment of forty thousand state-of-the-art solar harvester sail cells in orbit around the primary solar grid Delta-9. 

The cells automatic-tracking algorithm aligned each mirror in correct geometry, feeding several terrawatts of power straight to the orbit storage array. The output will fuel neighboring colony structures and orbital server farms. Phase 5 is planned to commence after local orbit alignment tests are carried out next month.`
  }
];

// Interactive Sports Updates
const SPORTS_UPDATES = [
  {
    id: 's1',
...
// Interactive Entertainment Headlines
const ENTERTAINMENT_ARTICLES = [
  {
    id: 'e1',
            ...
            >
              ...
              {activeTab === 'for_you' && (
                <>
                  ...
                  <div 
                    onClick={() => openNewsDetails(NEWS_ARTICLES[0])}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/20 to-transparent flex flex-col justify-end p-5 md:p-6 text-left">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] text-white/50">{NEWS_ARTICLES[0].time}</span>
                      </div>
                  ...
                  <div className="space-y-4">
                    {NEWS_ARTICLES.map((article) => (
                      <div 
                        ...
                        className="flex flex-col sm:flex-row gap-4 p-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-3xl cursor-pointer transition-all group hover:border-indigo-500/20"
                      >
              ...
              {activeTab === 'entertainment' && (
                <div className="space-y-5 text-left">
                  ...
                  <div className="space-y-4">
                    {ENTERTAINMENT_ARTICLES.map((article) => (
                      <div 
                        key={article.id}
                        onClick={() => openNewsDetails(article)}
                        className="flex flex-col sm:flex-row gap-4 p-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-3xl cursor-pointer transition-all group hover:border-[#00ffd5]/20"
                      >
                        <div className="w-full sm:w-36 h-28 shrink-0 rounded-2xl overflow-hidden bg-slate-800 border border-white/5">
                          <img src={article.image} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        </div>
